...
const PopularCardTvShows = ({poster, title, year, overview, ratings, id, data} : PopularProps) => {
  ...
  return (
      ...
      <div className='relative w-40 h-52 md:w-52 md:h-72 hover:scale-110 duration-300'>
            <Image className='w-full h-full rounded-lg' width={224} height={288} src={poster === null ? notFound : `https://image.tmdb.org/t/p/w500${poster}` || notFound } alt=''/>  
           ...
           :
              <button onClick={() => addToWatchListTv(id, user?.id)} className='flex justify-end w-full'>
