    ...
    <AuthBackground>
      <div className='absolute w-full h-full top-0 bg-black/50'>
        ...
        <div className='flex justify-center items-center h-[60%]'>
          <form className='bg-black/80 rounded-xl p-6 w-[500px] space-y-5' onSubmit={handleSubmit(handleLogin)}>
            <h1 className='text-center font-bold text-4xl'>Log In</h1>

            <div>
              <label htmlFor="email" className='block mb-2'>Email</label>
              <input {...register('email')} className='w-full bg-transparent border rounded-lg py-2 px-4' type="email" name="email" placeholder='Your Email' id='email' />
              {errors.email && <p className="text-red-600 mt-1">{errors.email.message}</p>}
            </div>

            <div>
              <label htmlFor="password" className='block mb-2'>Password</label>
              <input {...register('password')} className='w-full bg-transparent border rounded-lg py-2 px-4' type="password" name="password" placeholder='Your Password' id='password' />
              {errors.password && <p className="text-red-600 mt-1">{errors.password.message}</p>}
            </div>

            <h1 className='text-center text-xl font-bold'>OR</h1>
