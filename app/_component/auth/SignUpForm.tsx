  ...
  return (
    <AuthBackground>
        ...
        <div className='container mx-auto'>
          <svg className='fill-red-600 w-40 h-40' xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 48 48">
        ...
        <div className='flex justify-center items-center h-[60%]'>
          <form className='bg-black/80 rounded-xl p-6 w-[500px] space-y-5' onSubmit={handleSubmit(handleCreate)}>
            <h1 className='text-center font-bold text-4xl'>Sign Up</h1>

            <div>
              <label htmlFor="username" className='block mb-2'>UserName</label>
              <input {...register('username')} className='w-full bg-transparent border rounded-lg py-2 px-4' type="text" placeholder='Your UserName' id='username' />
              {errors.username && <p className='text-red-600 mt-1'>{errors.username.message}</p>}
            </div>
          
            <div>
              <label htmlFor="email" className='block mb-2'>Email</label>
              <input {...register('email')} className='w-full bg-transparent border rounded-lg py-2 px-4' type="email" placeholder='Your Email' id='email' />
              {errors.email && <p className='text-red-600 mt-1'>{errors.email.message}</p>}
            </div>

            <div>
              <label htmlFor="password" className='block mb-2'>Password</label>
              <input {...register('password')} className='w-full bg-transparent border rounded-lg py-2 px-4' type="password" placeholder='Your Password' id='password' />
              {errors.password && <p className='text-red-600 mt-1'>{errors.password.message}</p>}
            </div>

            <div>
              <label htmlFor="confirmpassword" className='block mb-2'>Confirm Password</label>
              <input {...register('confirmpassword')} className='w-full bg-transparent border rounded-lg py-2 px-4' type="password" placeholder='Confirm Your Password' id='confirmpassword' />
